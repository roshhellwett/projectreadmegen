# src/projectreadmegen/ai_provider.py
#
# Groq LPU AI integration for README generation.
# Named ai_provider.py (not grok/groq) so it can support multiple providers later.
...
class GroqClient:
    """Client for the Groq LPU inference API (OpenAI-compatible)."""
    ...
    def generate_graph_chat(
        ...
        """
        Send multi-turn chat messages with graph & code context and return grounded AI reply.
        """
        if not self.api_key:
            raise CustomAPIError(
                "API key not set",
                "API key not configured. Set GROQ_API_KEY environment variable or add via Dashboard.",
            )
...
# ---------------------------------------------------------------------------
# Backwards-compatible aliases for old ``grok`` naming
# ---------------------------------------------------------------------------
